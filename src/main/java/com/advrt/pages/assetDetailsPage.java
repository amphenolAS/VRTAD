...
// Check the presence of Setup tile
	...
	// Click on Setup header text block
		public void Click_SetupsHeader() {
...
// Check the presence of Delete Button in set up section
	...
	// Click the Delete Button in set up section
...
// Check the presence of Sop Number Field
	public boolean IsSOPNumberField_Visible() {
...
// check the presence of GenerateReportsForQualButton
	...
	// Check the presence of Copytodrive btn
		public boolean Is_Copytodrive_Visible() throws InterruptedException {
			WebElement CopytodriveBtn = driver.findElementByName("Copy to drive");
			return IsElementVisibleStatus(CopytodriveBtn);
		}
...
// Check the presence of Setup Reports Button under report section
	...
	// Check the presence of Setup Reports Button under report section
	public boolean GraphReportsButton_State() {
		WebElement GraphReports_Btn = driver.findElementByAccessibilityId("graphReportsButton");
		return IsElementVisibleStatus(GraphReports_Btn);
	}

	// click on the setup sub section from Reports tile
	public void Click_GraphReportsButton() {
		WebElement GraphReports_Btn = driver.findElementByAccessibilityId("graphReportsButton");
...
// Check the presence of ReportsHeaderTextBlock
	...
	// Click on  ReportsHeaderTextBlock
		public void Click_ReportsHeaderText_ReportsTab() {
			WebElement ReportsHeader_TextBlock = driver.findElementByAccessibilityId("ReportsHeaderTextBlock");
...
// Click new Setup create button
	...
	public void Click_CopyStupBtn() throws InterruptedException, IOException {
		clickOn(CopySetup_Btn);
		Thread.sleep(500);
	}
...
// Click VRT app close button
	...
	public void click_vrtAppcloseBtn() throws InterruptedException {
		WebElement VRTappClose_Btn = driver.findElementByAccessibilityId("Close");
...
// Click on "Yes" button from the delete confirmation pop-up for asset 
	...
	// Click on Copy asset button with Alert
	public void click_Copyasset_Alert() throws InterruptedException, IOException {
		clickOn(CopyAsset);
		Thread.sleep(500);
	}
...
// ||Hard code || Select and click on the mentioned set up file
	...
	public void ClickSetup_file() throws InterruptedException, IOException {
		WebElement Setupname = driver.findElementByName("Qual_case_-51");
		clickOn(Setupname);
	}
...
// Loop for the different serial number created
	...
	//Select Detailed Report based on Date (DD-MMM-YYYY) format
	public void Select_DetailedReport(String FileDate) throws InterruptedException, IOException {
		WebElement FilesList = driver.findElementByClassName("ListView");
		clickOn(FilesList);
		...
		for (int i = 0; i < DRList.size(); i++) {
			List<WebElement> D_REPORTList = DRList.get(i).findElements(By.className("TextBlock"));			

			for (int j = 0; j < D_REPORTList.size(); j++) {
				//DREPORTList.get(0).click();
				String rt = D_REPORTList.get(j).getText();
				//System.out.println(rt);
				if (rt.contains(FileDate)) {
					D_REPORTList.get(j).click();
					break;
				}
			}
		}
	...
	// Select Summary Report based on Date (DD-MMM-YYYY) format
	public void Select_SummaryReport(String FileDate) throws InterruptedException, IOException {
		WebElement FilesList = driver.findElementByClassName("ListView");
		clickOn(FilesList);
		...
		for (int i = 0; i < SRList.size(); i++) {
			List<WebElement> SREPORTList = SRList.get(i).findElements(By.className("TextBlock"));

			for (int j = 0; j < SREPORTList.size(); j++) {
				//SREPORTList.get(0).click();
				String rt = SREPORTList.get(j).getText();
				if (rt.equals(FileDate)) {
					SREPORTList.get(j).click();
					break;
				} 
			}
		}
	...
	//Select Pass Fail Report
	public void Select_PFReport(String FileDate) throws InterruptedException, IOException {
		...
		// Loop for the different serial number created
		for (int i = 0; i < DRList.size(); i++) {
			List<WebElement> PR_REPORTList = DRList.get(i).findElements(By.className("TextBlock"));

			for (int j = 0; j < PR_REPORTList.size(); j++) {
				//PR_REPORTList.get(0).click();
				String rt = PR_REPORTList.get(j).getText();
				//System.out.println(rt);
				if (rt.equals(FileDate)) {
					PR_REPORTList.get(j).click();
					break;
				}
			}
		}
...
//Click on Ok button under delete login pop upm (Not able to delete the asset with files)
	...
	public void ClickOK_btn() throws InterruptedException, AWTException, IOException {
