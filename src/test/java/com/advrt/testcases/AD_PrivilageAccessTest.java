...
public class AD_PrivilageAccessTest extends BaseClass {
	...
	private void PreSetUp() throws Exception {
		...
		extent = new ExtentReports(
				System.getProperty("user.dir") + "/test-output/ER" + "_AD_PrivilageAccessTestReg(1.6.14)" + ".html",
				true);
		...
		Thread.sleep(500);
		UserLoginPopup_UserCommentTextBox(prop.getProperty("Group1UserId"), prop.getProperty("Group1Pwd"),
				"usercommitted.");
	...
	// PA001-Verify if User able to access the User Management Module when the User
	// Management Privilege is checked
	...
	public void PA001() throws InterruptedException, IOException {
		...
		sa.assertEquals(AD_UMPage.UMtabPresence(), true, "Fail: AdminUmPriv is Not checked");

		sa.assertAll();
	}

	// PA003-Verify if User able to access the Run Qualification Module when the Run
	// Qualification Privilege is checked
	...
	public void PA003() throws InterruptedException, IOException, ParseException {
		extentTest = extent.startTest(
		...
		// Define Setup
				...
				Setup_ReviewPage.click_Save_Btn("Manual", "Yes", prop.getProperty("Group1UserId"), prop.getProperty("Group1Pwd"),"usercommitted.");
	...
	// PA005-Verify if User able to access the Run Calibration Module when the Run
	// Calibration Privilege is checked
	...
	public void PA005() throws InterruptedException, IOException {
		...
		// DefaultUserPrivilages_page.Click_Create_UserManagement();while running
		// individually must uncommit
		...
		String ExpectedMsg = " User Type : \"NUser\" , User Privileges : \"Run Calibration\" created by User ID : \"Ajay2\" , User Name : \"Ajay Mashal\"";

		sa.assertEquals(ActualMsg, ExpectedMsg, "Fail:User unable to access Run calibration even if it is checked");

		sa.assertAll();
	}

	// PA007-Verify if User able to access the Run Verification Module when the Run
	// Verification Privilege is checked
	...
	public void PA007() throws InterruptedException, IOException {
		...
		String ActualMsg = AuditPage.get_auditEvent_text();
		String ExpectedMsg = " User Type : \"User1\" , User Privileges : \"Run Verification\" created by User ID : \"Ajay2\" , User Name : \"Ajay Mashal\"";

		sa.assertEquals(ActualMsg, ExpectedMsg, "Fail:User unable to access Run Verification even if it is checked");

		sa.assertAll();
	}

	// PA009-Verify if User able to access the Create Assets Module when the Create
	// Assets Privilege is checked
	...
	public void PA009() throws InterruptedException, IOException, ParseException {
		...
		Thread.sleep(1000);
		AD_UMPage.Select_grp();
		AD_UMPage.select_2grp(prop.getProperty("Group2"));
		AD_UMPage.select_UserTitle("Manager");
		...
		MainHubPage = AD_UMPage.click_BackBtn();
		MainHubPage.UserSignOut();
		// as we have assigned newusertype to QATesters2,we are using that user which
		// are available in QATesters2
		LoginPage = new LoginPage();
		...
		assetCreationPage.assetCreationWithAllFieldEntry("Asset01", "01", "HeatBath", "Aas", "Hyderabad", "VRT-RF", "2",
				"cu", crntDate, "5", "Weeks", "1st Asset Creation");
		UserLoginPopup_UserCommentTextBox(prop.getProperty("Group2UserId"), prop.getProperty("Group2Pwd"),
				"usercommitted.");
		Thread.sleep(1000);
		assetHubPage = assetCreationPage.clickBackBtn();
		assetDetailsPage = assetHubPage.click_assetTile("Asset01");
	...
	// PA011-Verify if User able to access the Modify Assets Module when the Modify
	// Assets Privilege is checked
	...
	public void PA0011() throws InterruptedException, IOException, ParseException {
		...
		sa.assertEquals(NewassetID, After_Clicking_Back, "FAIL:The modified values is not displayed in the field");

		sa.assertAll();
	}

	// PA013-Verify if User able to access the Delete Assets Module when the Delete
	// Assets Privilege is checked
	...
	public void PA0013() throws InterruptedException, IOException, ParseException, AWTException {
		...
		assetCreationPage = assetHubPage.ClickAddAssetBtn();
		String crntDate = tu.get_CurrentDate_inCertainFormat("MM/dd/YYYY");
		assetCreationPage.assetCreationWithAllFieldEntry("Asset033", "03", "HeatBath", "Aas", "Hyderabad", "VRT-RF",
		...
		String ExpectedMsg = "Asset: \"Asset033\" is deleted by User Id : \"Ajay2\" , User Name : \"Ajay Mashal\"";

		sa.assertEquals(ActualMsg, ExpectedMsg, "Fail:Asset is not deleted");

		// sa.assertEquals(assetDetailsPage.assetDetailPageTitle_Visible(), false, "Fail
		// : Page title is still displayed");
		// sa.assertEquals(assetHubPage.Is_assetHubPageTitle_Visible(), true, "Fail :
	...
	/*
	 * //PA015-Verify if User able to access the Copy Files/Reports Module when the
	 * Copy Files/Reports Privilege is checked
	 * 
	 * @Test(groups = { "Sanity", "Regression" }, description =
	 * "PA015-Verify if User able to access the Copy Files/Reports Module when the Copy Files/Reports Privilege is checked"
	 * )
	 * 
	 * public void PA0015() throws InterruptedException, IOException { extentTest =
	 * extent
	 ...
	 * ); SoftAssert sa = new SoftAssert();
	 * 
	 * AD_UMPage.Select_grp(); AD_UMPage.select_grp(prop.getProperty("Group1"));
	 * AD_UMPage.Select_user(); AD_UMPage.select_user(1);
	 * AD_UMPage.select_UserTitle("Manager");
	 * AD_UMPage.select_UserType1("NewUserType");
	 * DefaultUserPrivilages_page=AD_UMPage.newUserType("User");
	 * 
	...
	// PA017-Verify if User able to access the Create Setups Module when the Create
	// Setups Privilege is checked
	...
	public void PA0017() throws InterruptedException, IOException, ParseException {
		...
		Setup_QualParamPage = Setup_CalculationsPage.Click_NxtBtn();
		Setup_ReviewPage = Setup_QualParamPage.Click_NxtBtn();
		Setup_ReviewPage.click_Save_Btn("Manual", "Yes", prop.getProperty("Group2UserId"),
				prop.getProperty("Group2Pwd"), "usercommitted.");
		assetDetailsPage = Setup_ReviewPage.click_backBtn();
		Thread.sleep(2000);
	...
	// PA019-Verify if User able to access the Modify Setups Module when the Modify
	// Setups Privilege is checked
	...
	// PA021-Verify if User able to access the Delete Setups Module when the Delete
	// Setups Privilege is checked
	...
	public void PA0021() throws InterruptedException, IOException, ParseException {
		...
		String ExpectMSG = "Setup : \"SetUp2\"  deleted by User ID : \"Ajay2\", User Name : \"Ajay Mashal\"";
	...
	// PA045-Verify if User able to access the Audit Trail Module when the Audit
	// Trail Privilege is checked
	...
	// PA047-Verify if User able to access the Policies Module when the Policies
	...
	// PA050-Verify if User able to access the Preferences Module when the
	// Preferences Privilege is checked
	...
	// PA025-Verify if User able to access the Create Equipment Module when the
	// Create Equipment Privilege is checked
...
//Individually getting passed
	// PA027-Verify if User able to access the Modify Equipment Module when the
	// Modify Equipment Privilege is checked
	...
	public void PA0027() throws InterruptedException, IOException, ParseException {
		...
		// DefaultUserPrivilages_page.Click_Create_UserManagement();//while running
		// individually must uncommit
		// DefaultUserPrivilages_page.Click_Create_Equipmet();// while running
		// individually must uncommit
...
//Individually getting passed
	// PA029-Verify if User able to access the Delete Equipment Module when the
	// Delete Equipment Privilege is checked
	...
	public void PA0029() throws InterruptedException, IOException, ParseException {
		...
		// DefaultUserPrivilages_page.Click_Create_Equipmet();//while running
		// individually must uncommit
		// DefaultUserPrivilages_page.Click_ModifyEquipment();//while running
		// individually must uncommit
		...
		UserLoginPopup_UserCommentTextBox(prop.getProperty("Group2UserId"), prop.getProperty("Group2Pwd"),
				"usercommitted.");
		Thread.sleep(2000);
		sa.assertFalse(EquipmentHubPage.is_ReqEpuipmentVisible("EL011b"),
				"Fail: Application still showing Equipment after deleting");
		sa.assertAll();
...
//PA037-Verify if User able to access the Camera Access Module when the Camera Access Privilege is checked 
	...
	public void PA037() throws InterruptedException, IOException, ParseException {
		extentTest = extent.startTest(
				"PA037-Verify if User able to access the Camera Access Module when the Camera Access Privilege is checked s");
		SoftAssert sa = new SoftAssert();
		...
		String crntDate = tu.get_CurrentDate_inCertainFormat("MM/dd/YYYY");
		assetCreationPage.assetCreationWithAllFieldEntry("Asset077", "07", "HeatBath", "Aas", "Hyderabad", "VRT-RF",
				"2", "cu", crntDate, "5", "Weeks", "1st Asset Creation");
		UserLoginPopup_UserCommentTextBox(prop.getProperty("Group2UserId"), prop.getProperty("Group2Pwd"),
				"usercommitted.");
		Thread.sleep(1000);
...
//PA031-Verify if User able to access the Manual Sync Module when the Manual Sync Privilege is checked
	...
	public void PA031() throws InterruptedException, IOException, ParseException, AWTException {
		extentTest = extent.startTest(
				"PA031-Verify if User  able to access the Manual Sync Module when the Manual Sync Privilege is unchecked");
		SoftAssert sa = new SoftAssert();
		...
		SyncInPage = FileManagementPage.AD_ClickSyncInBtn_SyncinPage_withcommit(prop.getProperty("Group1UserId"),
				prop.getProperty("Group1Pwd"), "usercommited");
...
//PA051-Verify if User able to access the Hardware Maintenance Module when the Hardware Maintenance Privilege is checked 
	...
	/*
	 * CRT Dependent //PA033-Verify if User able to access the Create Reports Module
	 * when the Create Reports Privilege is checked
	 * 
	 * @Test(priority=19,groups = { "Sanity", "Regression" }, description =
	 * "PA033-Verify if User able to access the Create Reports Module when the Create Reports Privilege is checked "
	 * )
	 * 
	 * public void PA033() throws InterruptedException, IOException, ParseException,
	 * AWTException { extentTest = extent
	 ...
	 * AD_UMPage.select_UserTitle("Manager");
	 * AD_UMPage.select_UserType1("NewUserType");
	 * DefaultUserPrivilages_page=AD_UMPage.newUserType("User"); Thread.sleep(500);
	 * //DefaultUserPrivilages_page.Click_Create_UserManagement();//while running
	 * individually must uncommit DefaultUserPrivilages_page.Click_Create_Reports();
	 * DefaultUserPrivilages_page.Click_ManualSync();///while running individually
	 * must uncommit DefaultUserPrivilages_page.NewSaveButton();
	 * UserLoginPopup_UserCommentTextBox(prop.getProperty("Group1UserId"),prop.
	 * getProperty("Group1Pwd"),"usercommitted."); Thread.sleep(1000);
	 * AD_UMPage.Select_grp(); AD_UMPage.select_2grp(prop.getProperty("Group2"));
	 * AD_UMPage.select_UserTitle("Manager"); AD_UMPage.select_UserType1("User");
	 * Thread.sleep(500); AD_UMPage.clickSavebtn();
	 * UserLoginPopup_UserCommentTextBox(prop.getProperty("Group1UserId"),prop.
	 * getProperty("Group1Pwd"),"usercommitted."); tu.click_OK_popup();
	 * MainHubPage=AD_UMPage.click_BackBtn(); MainHubPage.UserSignOut(); //as we
	 * have assigned newusertype to QATesters2,we are using that user which are
	 * available in QATesters2 LoginPage = new LoginPage(); MainHubPage =
	 * LoginPage.Login(prop.getProperty("Group2UserId"),prop.getProperty("Group2Pwd"
	 * )); FileManagementPage = MainHubPage.ClickFileManagementTitle(); SyncInPage =
	 * FileManagementPage.AD_ClickSyncInBtn_SyncinPage_withcommit(prop.getProperty(
	 * "Group1UserId"),prop.getProperty("Group1Pwd"),"usercommited");
	 * SyncInPage.enter_Filepath("syncin"); SyncInPage.click_FltrBtn();
	 * SyncInAssetListPage = SyncInPage.click_SyncInOK_btn();
	 * SyncInAssetListPage.click_EquipmentCheckBox();
	 * SyncInAssetListPage.click_SelectAllBtn(); SyncInAssetListPage.click_OkBtn();
	 * SyncInAssetListPage.click_AlrtYesBtn(); Thread.sleep(9000);
	 * SyncInAssetListPage.click_Success_alrtMeg_OkBtn(); Thread.sleep(2000);
	 * 
	 * 
	 * LaunchApp("Kaye.ValProbeRT_racmveb2qnwa8!App"); Thread.sleep(500); LoginPage
	 * = new LoginPage(); MainHubPage =
	 * LoginPage.Login(prop.getProperty("Group2UserId"),prop.getProperty("Group2Pwd"
	 * ));
	 * 
	 * assetHubPage=MainHubPage.Click_AssetTile2(); assetDetailsPage =
	 * assetHubPage.click_assetTile("SyncInAsset");
	 ...
	 * "Fail: Guest unable to access Verification module  "); sa.assertAll();
	 * 
	 * 
	 * }
	 */
...
//PA035-Verify if User able to access the Delete StudyFiles/Reports Module when the Delete StudyFiles/Reports Privilege is checked 
	...
	public void PA035() throws InterruptedException, IOException, ParseException, AWTException {
		...
		UserLoginPopup_UserCommentTextBox(prop.getProperty("Group2UserId"), prop.getProperty("Group2Pwd"), "committed");
		Thread.sleep(2000);
		...
		String ExpectMSG = "Qualification Study : \"manual 1 min sampling\"  deleted by User ID : \"t1\", User Name : \"Test1\"";
		sa.assertEquals(Actionmsg, ExpectMSG,
